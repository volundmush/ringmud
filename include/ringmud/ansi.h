...
#ifndef RINGMUD_ANSI_H
#define RINGMUD_ANSI_H
...
namespace ring::ansi {
    ...
    extern std::map<uint8_t, std::string> default_choices, empty_choices;

    std::string processColors(const std::string& txt, bool parse, std::map<uint8_t, std::string>& choices = empty_choices);

    std::string stripAnsi(const std::string& txt);
}

#endif //RINGMUD_ANSI_H
