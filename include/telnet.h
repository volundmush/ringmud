...
namespace ring::telnet {
    ...
    public:
        ...
        bool startWill() const, startDo() const, supportLocal() const, supportRemote() const, isReady() const;
        void registerHandshake();
        void enableLocal(), enableRemote(), disableLocal(), disableRemote();
        void negotiate(uint8_t command);
    ...
    class TelnetProtocol {
    public:
        explicit TelnetProtocol(entt::entity ent);
        ...
        void checkReady(), start(), onConnect();
        bool isReady();
        entt::entity entity;
    private:
        void sendString();
        ...
        time_t start_time{}; // used for detecting when a connection is ready
    };
}
